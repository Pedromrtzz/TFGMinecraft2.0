package com.pedromrtz.tfgmod.network;

import com.pedromrtz.tfgmod.progress.Chapter1ProgressProvider;
import net.minecraft.network.FriendlyByteBuf;
import net.minecraft.network.chat.Component;
import net.minecraft.server.level.ServerPlayer;

public class CompleteChapter5PuzzleC2SPacket {

    public CompleteChapter5PuzzleC2SPacket() {}

    public static void encode(CompleteChapter5PuzzleC2SPacket msg, FriendlyByteBuf buf) {}

    public static CompleteChapter5PuzzleC2SPacket decode(FriendlyByteBuf buf) {
        return new CompleteChapter5PuzzleC2SPacket();
    }

    public static void handle(CompleteChapter5PuzzleC2SPacket msg, Object ctxObj) {

        try {

            var ctx = ctxObj;

            var enqueueWork =
                    ctx.getClass().getMethod("enqueueWork", Runnable.class);

            var getSender =
                    ctx.getClass().getMethod("getSender");

            var setHandled =
                    ctx.getClass().getMethod("setPacketHandled", boolean.class);

            enqueueWork.invoke(ctx, (Runnable) () -> {

                try {

                    ServerPlayer sp =
                            (ServerPlayer) getSender.invoke(ctx);

                    if (sp == null) return;

                    sp.getCapability(
                            Chapter1ProgressProvider.CHAPTER1_PROGRESS
                    ).ifPresent(progress -> {

                        progress.setChapter5Task(3);

                        sp.displayClientMessage(
                                Component.literal(
                                        "§aThe inner castle gate opens."
                                ),
                                false
                        );

                        ProgressSync.syncChapter1(sp);
                    });

                } catch (Exception ignored) {}
            });

            setHandled.invoke(ctx, true);

        } catch (Exception ignored) {}
    }
}